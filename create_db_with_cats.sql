...
(
    ...
    type ENUM("Hard Serve", "Gelato", "Sherbet", "Sorbet", "Frozen Yogurt", "Soft Serve", "Other"),
    in_stock BOOLEAN,
    chain_ID INT,
...
(
	cat_name VARCHAR(64) PRIMARY KEY,
...
DROP TABLE IF EXISTS category_mixin;
CREATE TABLE category_mixin
(
	cat_name VARCHAR(64) NOT NULL,
    mixin_name VARCHAR(64) NOT NULL,
    UNIQUE(cat_name, mixin_name),
    CONSTRAINT cat_to_mixin_fk
		FOREIGN KEY (cat_name)
        REFERENCES category(cat_name)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,
    CONSTRAINT mixin_to_cat_fk
		FOREIGN KEY (mixin_name)
        REFERENCES mixin(mixin_name)
        ON DELETE RESTRICT
        ON UPDATE CASCADE
...
(
	cat_name VARCHAR(64) NOT NULL,
    base_name VARCHAR(64) NOT NULL,
    UNIQUE(cat_name, base_name),
    CONSTRAINT cat_to_base_fk
		FOREIGN KEY (cat_name)
        REFERENCES category(cat_name)
        ON DELETE RESTRICT
        ON UPDATE CASCADE,
    CONSTRAINT base_to_cat_fk
		FOREIGN KEY (base_name)
        REFERENCES base(base_name)
        ON DELETE RESTRICT
        ON UPDATE CASCADE
